...
export const ComposerCommandPopover = memo(function ComposerCommandPopover(
  ...
  const renderItem = useCallback(
    ({ item, index }: { readonly item: ComposerCommandItem; readonly index: number }) => (
      <CommandRow item={item} onSelect={props.onSelect} isLast={index === props.items.length - 1} />
    ),
    [props.items.length, props.onSelect],
  );

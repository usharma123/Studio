...
import { MetaCard, SheetListRow } from "./gitSheetComponents";
import { menuItemIconName, statusSummary } from "./gitSheetUtils";
...
function useGitOverviewSheetModel(props: GitOverviewSheetProps) {
...
export function GitOverviewSheet(props: GitOverviewSheetProps): React.JSX.Element {
  const screenModel = useGitOverviewSheetModel({ ...props });
  if (screenModel.kind === "terminal") return screenModel.content;
  return <GitOverviewSheetView model={screenModel} />;
}

function GitOverviewSheetView({
  model,
}: {
  readonly model: Extract<ReturnType<typeof useGitOverviewSheetModel>, { readonly kind: "ready" }>;
}) {
  const {
    busy,
    content,

...
import { useNewTaskFlow } from "./newTaskFlowContext";
import { branchBadgeLabel } from "./newTaskFlowModel";
...
function useNewTaskDraftScreenModel(props: {
  ...
  return {
    ...
    environmentMenuActions,
    flow,
    handleEnvironmentMenuAction,
...
export function NewTaskDraftScreen(props: {
  ...
  const screenModel = useNewTaskDraftScreenModel(props);
  if (screenModel.kind === "terminal") return screenModel.content;
  return <NewTaskDraftScreenView model={screenModel} />;
}

function NewTaskDraftScreenView({
  model,
}: {
  readonly model: Extract<
    ReturnType<typeof useNewTaskDraftScreenModel>,
    { readonly kind: "ready" }
  >;
}) {
  const {

...
import { SwipeableScrollGateProvider } from "../home/thread-swipe-actions";
import { useSwipeableScrollGate } from "../home/useSwipeableScrollGate";
...
function useThreadNavigationSidebarPaneModel(
...
function ThreadNavigationSidebarPane(
  ...
  const screenModel = useThreadNavigationSidebarPaneModel({ ...props });
  if (screenModel.kind === "terminal") return screenModel.content;
  return <ThreadNavigationSidebarPaneView model={screenModel} />;
}

function ThreadNavigationSidebarPaneView({
  model,
}: {
  readonly model: Extract<
    ReturnType<typeof useThreadNavigationSidebarPaneModel>,
    { readonly kind: "ready" }
  >;
}) {
  const {

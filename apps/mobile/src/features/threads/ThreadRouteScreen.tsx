...
} from "../layout/adaptiveWorkspaceContext";
...
function useThreadRouteContentModel(
  ...
  const threadGitControlProps = {
    ...
    availableTargets: selectedThreadProject?.workspaceRoot
      ? (["files", "terminal"] as const)
      : ([] as const),
  ...
  if (!environmentId || !threadId) {
    return { kind: "terminal", content: <OpeningThreadLoadingScreen /> } as const;
  }
  ...
  const renderThreadRouteBody = (showActionControls: boolean) => (
    <>
      <ThreadGitControls
        ...
        displayMode={showActionControls ? (layout.usesSplitView ? "split" : "compact") : "hidden"}
      />
...
function ThreadRouteContent(
  ...
  const screenModel = useThreadRouteContentModel({ ...props });
  if (screenModel.kind === "terminal") return screenModel.content;
  return <ThreadRouteContentView model={screenModel} />;
}

function ThreadRouteContentView({
  model,
}: {
  readonly model: Extract<
    ReturnType<typeof useThreadRouteContentModel>,
    { readonly kind: "ready" }
  >;
}) {
  const {

...
import { ActivityIndicator, Pressable, useColorScheme, View, type ViewStyle } from "react-native";
...
function useThreadComposerModel(props: ThreadComposerProps) {
  ...
  const [previewImageUri, dispatchPreviewImageUri] = useReducer(
  ...
  const onPressImage = useCallback(
    (uri: string) => {
      ...
      dispatchPreviewImageUri(uri);
    },
    [isFocused],
  );

  const closePreview = useCallback(() => {
    dispatchPreviewImageUri(null);
  ...
  // ── Trigger detection ────────────────────────────────────
  const [composerSelection, dispatchComposerSelection] = useReducer(
    (
      current: ComposerEditorSelection,
      action: ComposerEditorSelection | { readonly clampTo: number },
    ): ComposerEditorSelection => {
      if (!("clampTo" in action)) {
        return action;
      }
      const start = Math.min(current.start, action.clampTo);
      const end = Math.min(current.end, action.clampTo);
      return start === current.start && end === current.end ? current : { start, end };
    },
    { start: props.draftMessage.length, end: props.draftMessage.length },
  );

  const handleSelectionChange = useCallback((selection: ComposerEditorSelection) => {
    dispatchComposerSelection(selection);
  }, []);
  useEffect(() => {
    dispatchComposerSelection({ clampTo: props.draftMessage.length });
  ...
  const handleCommandSelect = useCallback(
    (item: ComposerCommandItem) => {
      ...
      if (
        ...
        dispatchComposerSelection({ start: result.cursor, end: result.cursor });
...
export const ThreadComposer = memo(function ThreadComposer(
  ...
  const screenModel = useThreadComposerModel({ ...props });
  return <ThreadComposerView model={screenModel} />;
});

function ThreadComposerView({
  model,
}: {
  readonly model: Extract<ReturnType<typeof useThreadComposerModel>, { readonly kind: "ready" }>;
}) {
  const {
    bodyText,
    canSend,

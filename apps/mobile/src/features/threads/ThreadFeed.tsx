...
import { hasNativeSelectableMarkdownText } from "../../native/SelectableMarkdownAvailability";
import {
...
import { useAppearancePreferences } from "../settings/appearance/appearancePreferencesContext";
...
function MessageAttachmentImage(props: {
  ...
  return (
    <Pressable className="active:opacity-70" onPress={() => props.onPressImage(uri)}>
      <Image source={{ uri }} className={props.className} contentFit="cover" />
    </Pressable>
  );
}
...
interface ThreadFeedInteractionState {
  readonly copiedRowId: string | null;
...
type ThreadFeedInteractionAction =
  | { readonly type: "set-copied-row"; readonly rowId: string | null }
  | { readonly type: "clear-copied-row"; readonly rowId: string }
  | { readonly type: "toggle-work-group"; readonly groupId: string }
  | { readonly type: "toggle-work-row"; readonly rowId: string }
  | { readonly type: "expand-turn"; readonly turnId: TurnId }
  | { readonly type: "collapse-turn"; readonly turnId: TurnId }
  | { readonly type: "toggle-turn"; readonly turnId: TurnId };

const initialThreadFeedInteractionState: ThreadFeedInteractionState = {
  copiedRowId: null,
  expandedWorkGroups: {},
  expandedWorkRows: {},
...
function reduceThreadFeedInteraction(
  state: ThreadFeedInteractionState,
  action: ThreadFeedInteractionAction,
): ThreadFeedInteractionState {
  switch (action.type) {
    case "set-copied-row":
      return { ...state, copiedRowId: action.rowId };
    case "clear-copied-row":
      return state.copiedRowId === action.rowId ? { ...state, copiedRowId: null } : state;
    case "toggle-work-group":
      return {
        ...state,
        expandedWorkGroups: {
          ...state.expandedWorkGroups,
          [action.groupId]: !(state.expandedWorkGroups[action.groupId] ?? false),
        },
      };
    case "toggle-work-row":
      return {
        ...state,
        expandedWorkRows: {
          ...state.expandedWorkRows,
          [action.rowId]: !(state.expandedWorkRows[action.rowId] ?? false),
        },
      };
    case "expand-turn":
      return {
        ...state,
        expandedTurnIds: new Set(state.expandedTurnIds).add(action.turnId),
      };
    case "collapse-turn": {
      if (!state.expandedTurnIds.has(action.turnId)) return state;
    ...
    case "toggle-turn": {
      const expandedTurnIds = new Set(state.expandedTurnIds);
      if (expandedTurnIds.has(action.turnId)) {
        expandedTurnIds.delete(action.turnId);
      } else {
...
function dispatchDisclosureInteraction(
  dispatch: (action: ThreadFeedInteractionAction) => void,
  suspendScrollMaintenance: (anchorKey: string | null) => void,
  anchorKey: string,
  action: ThreadFeedInteractionAction,
): void {
  suspendScrollMaintenance(anchorKey);
  dispatch(action);
}

function useThreadFeedModel(props: ThreadFeedProps) {
  ...
  const [interactionState, dispatchInteraction] = useReducer(
    reduceThreadFeedInteraction,
    initialThreadFeedInteractionState,
  );
  ...
  useEffect(() => {
    ...
    if (props.latestTurn.turnId === previous.turnId) {
      if (previous.state === "running" && props.latestTurn.state === "interrupted") {
        ...
        dispatchInteraction({ type: "expand-turn", turnId: interruptedTurnId });
      }
      return;
    }
    dispatchInteraction({ type: "collapse-turn", turnId: previous.turnId });
  }, [props.latestTurn]);
  ...
  const onCopyWorkRow = useCallback((rowId: string, value: string) => {
    ...
    dispatchInteraction({ type: "set-copied-row", rowId });
    if (copyFeedbackTimeoutRef.current) {
      clearTimeout(copyFeedbackTimeoutRef.current);
    }
    copyFeedbackTimeoutRef.current = setTimeout(() => {
      dispatchInteraction({ type: "clear-copied-row", rowId });
  ...
  const onToggleWorkGroup = useCallback(
    (groupId: string) => {
      dispatchDisclosureInteraction(
        dispatchInteraction,
        suspendEndScrollMaintenanceForDisclosure,
        `work-toggle:${groupId}`,
        { type: "toggle-work-group", groupId },
      );
    },
    [suspendEndScrollMaintenanceForDisclosure],
  );
...
export const ThreadFeed = memo(function ThreadFeed(props: ThreadFeedProps): React.JSX.Element {
  const screenModel = useThreadFeedModel({ ...props });
  if (screenModel.kind === "terminal") return screenModel.content;
  return <ThreadFeedView model={screenModel} />;
});

function ThreadFeedView({
  model,
}: {
  readonly model: Extract<ReturnType<typeof useThreadFeedModel>, { readonly kind: "ready" }>;
}) {
  const {
    anchorTopInset,
    anchoredEndSpace,

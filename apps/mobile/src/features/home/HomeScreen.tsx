...
import { SwipeableScrollGateProvider } from "./thread-swipe-actions";
import { useSwipeableScrollGate } from "./useSwipeableScrollGate";
...
function useHomeScreenModel(props: HomeScreenProps) {
...
export function HomeScreen(props: HomeScreenProps): React.JSX.Element {
  const screenModel = useHomeScreenModel({ ...props });
  if (screenModel.kind === "terminal") return screenModel.content;
  return <HomeScreenView model={screenModel} />;
}

function HomeScreenView({
  model,
}: {
  readonly model: Extract<ReturnType<typeof useHomeScreenModel>, { readonly kind: "ready" }>;
}) {
  const {
    connectionStatus,
    extraData,

...
export function ConnectionsNewRouteScreen({
  ...
  const [{ hostInput, codeInput }, updatePairingInput] = useReducer(
    (
      state: { readonly hostInput: string; readonly codeInput: string },
      update: Partial<{ readonly hostInput: string; readonly codeInput: string }>,
    ) => ({ ...state, ...update }),
    { hostInput: "", codeInput: "" },
  );
  ...
  useEffect(() => {
    ...
    updatePairingInput({ hostInput: host, codeInput: code });
  }, [connectionPairingUrl]);
  ...
  const handleHostChange = useCallback((value: string) => {
    updatePairingInput({ hostInput: value });
  }, []);

  const handleCodeChange = useCallback((value: string) => {
    updatePairingInput({ codeInput: value });
  }, []);

...
function ReviewFileNavigator({
  ...
  const fileList = (
    <FlatList
      ...
      contentInset={{
        bottom: 8,
        top: Platform.OS === "ios" ? insets.top + 44 + 8 : 8,
      }}
...
function useReviewSheetModel(props: ReviewSheetProps) {
  ...
  return {
    ...
    gitState,
    gitStatusQuery,
    handleNativeToggleFile,
    handleNativeToggleViewedFile,
    handlePullToRefresh,
...
export function ReviewSheet(props: ReviewSheetProps): React.JSX.Element {
  const screenModel = useReviewSheetModel({ ...props });
  return <ReviewSheetView model={screenModel} />;
}

function ReviewSheetView({
  model,
}: {
  readonly model: Extract<ReturnType<typeof useReviewSheetModel>, { readonly kind: "ready" }>;
}) {
  const {
    NativeReviewDiffView,
    commentSelection,

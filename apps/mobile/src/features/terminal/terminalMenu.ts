...
export function buildTerminalMenuSessions(input: {
  ...
  for (const session of input.knownSessions) {
    const { status, summary, hasRunningSubprocess } = session.state;
    const { terminalId } = session.target;
    if (
      status !== "running" &&
      status !== "starting" &&
      terminalId !== input.currentSession?.terminalId
    ) {
      continue;
    }

    sessionsById.set(terminalId, {
      terminalId,
      cwd: summary?.cwd ?? input.workspaceRoot,
      status,
      hasRunningSubprocess,
      displayLabel: resolveTerminalSessionLabel(terminalId, summary),

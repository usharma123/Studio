...
import { useAdaptiveWorkspaceLayout } from "../layout/adaptiveWorkspaceContext";
...
function useFirstNonNull<T>(options: { readonly value: T | null }): T | null {
  const [firstValue, setFirstValue] = useState(options.value);
  if (firstValue === null && options.value !== null) {
    setFirstValue(options.value);
    return options.value;
  }
  return firstValue;
}

function useThreadTerminalRouteScreenModel(props: ThreadTerminalRouteScreenProps) {
  ...
  const launchTargetKey = launchTarget
    ...
    : null;
  const pendingLaunchEntryRef = useRef<{
    readonly launch: PendingTerminalLaunch | null;
  } | null>(null);
  if (pendingLaunchEntryRef.current === null) {
    pendingLaunchEntryRef.current = {
      launch: launchTarget === null ? null : takePendingTerminalLaunch(launchTarget),
    };
  }
  const pendingLaunchEntry = pendingLaunchEntryRef.current;
  const pendingLaunch = pendingLaunchEntry.launch;
  const hasResolvedPendingLaunch = true;
  const initialAttachGridEntryRef = useRef<{
    readonly size: { readonly cols: number; readonly rows: number };
  } | null>(null);
  if (initialAttachGridEntryRef.current === null) {
    initialAttachGridEntryRef.current = {
  ...
  const initialAttachGridEntry = initialAttachGridEntryRef.current;
  const initialAttachGridSize = initialAttachGridEntry.size;
  const lastGridSizeRef = useRef(
    cachedRouteGridSize ?? {
      cols: DEFAULT_TERMINAL_COLS,
      rows: DEFAULT_TERMINAL_ROWS,
    },
  ...
  const launchLocation = useFirstNonNull({ value: launchLocationCandidate });
  ...
  useEffect(() => {
    ...
    lastGridSizeRef.current = getCachedTerminalGridSize({
      environmentId: routeEnvironmentId,
      threadId: routeThreadId,
      terminalId,
    }) ?? {
  ...
  const handleResize = useCallback(
    (size: { readonly cols: number; readonly rows: number }) => {
      ...
      const previousGridSize = lastGridSizeRef.current;
      if (size.cols === previousGridSize.cols && size.rows === previousGridSize.rows) {
        return;
      }
  ...
  if (!selectedThread) {
    if (workspaceState.isLoadingConnections) {
      return { kind: "terminal", content: <LoadingScreen message="Opening terminal…" /> } as const;
    }
...
export function ThreadTerminalRouteScreen(
  ...
  const params = props.route.params;
  const routeKey = [
    firstRouteParam(params.environmentId),
    firstRouteParam(params.threadId),
    firstRouteParam(params.terminalId) ?? DEFAULT_TERMINAL_ID,
  ].join(":");
  const selectionKey = selectedThread
    ? `${selectedThread.environmentId}:${selectedThread.id}`
    : "pending-selection";

  return <ThreadTerminalRouteSession key={`${routeKey}:${selectionKey}`} {...props} />;
}

function ThreadTerminalRouteSession(props: ThreadTerminalRouteScreenProps): React.JSX.Element {
  const screenModel = useThreadTerminalRouteScreenModel({ ...props });
  if (screenModel.kind === "terminal") return screenModel.content;
  return <ThreadTerminalRouteScreenView model={screenModel} />;
}

function ThreadTerminalRouteScreenView({
  model,
}: {
  readonly model: Extract<
    ReturnType<typeof useThreadTerminalRouteScreenModel>,
    { readonly kind: "ready" }
  >;
}) {
  const {

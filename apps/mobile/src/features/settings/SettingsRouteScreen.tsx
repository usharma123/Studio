...
function useConfiguredSettingsRouteScreenModel() {
  ...
  return {
    ...
    deviceRegistered,
    environmentCount,
    handleDeviceNotificationsChange,
    handleLiveActivitiesChange,
    insets,
    isLoaded,
    liveActivityStatus,
    notificationStatus,
...
function ConfiguredSettingsRouteScreen(): React.JSX.Element {
  const screenModel = useConfiguredSettingsRouteScreenModel();
  return <ConfiguredSettingsRouteScreenView model={screenModel} />;
}

function ConfiguredSettingsRouteScreenView({
  model,
}: {
  readonly model: Extract<
    ReturnType<typeof useConfiguredSettingsRouteScreenModel>,
    { readonly kind: "ready" }
  >;
}) {
  const {

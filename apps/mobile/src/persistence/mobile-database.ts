...
async function migrateLegacyFileCaches(database: SQLiteDatabase): Promise<boolean> {
  try {
    ...
    const migrationResults = await Promise.all(
      LEGACY_CACHE_DIRECTORIES.map(async (directoryName) => {
        try {
          const directory = new Directory(Paths.document, directoryName);
          if (!directory.exists) return true;
          const records = (
            await Promise.all(
              listFiles(directory).map(async (file) =>
                decodeLegacyCacheRecord(directoryName, await file.text()),
              ),
            )
    ...
    return migrationResults.every(Boolean);
  } catch (cause) {

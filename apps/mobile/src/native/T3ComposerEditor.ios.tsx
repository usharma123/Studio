...
function fileIconUri(path: string): string {
  const source = markdownFileIconSource(resolveMarkdownFileIcon(path));
...
export function ComposerEditor({
  ...
  useEffect(() => {
    confirmedTokensRef.current = confirmedTokens;
  }, [confirmedTokens]);
  const tokensJson = useMemo(
    () =>
      JSON.stringify(
        confirmedTokens.map((token) => ({
          type: token.type,
          source: token.source,
          start: token.start,
          end: token.end,

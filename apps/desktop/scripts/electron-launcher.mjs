...
export function resolveDevelopmentUserDataPath({
  ...
  const directoryName = profileSlug ? `codex-studio-dev-${profileSlug}` : "codex-studio-dev";
  return path.join(appDataDirectory, directoryName);
}
...
export function makeDevelopmentLauncherScript({
  ...
  return [
    ...
    `exec ${shellSingleQuote(electronBinaryPath)} ${shellSingleQuote(`--user-data-dir=${userDataPath}`)} --t3code-dev-root=${shellSingleQuote(desktopRoot)} ${shellSingleQuote(mainEntryPath)} "$@"`,
    "",
  ].join("\n");
}
...
function buildMacLauncher(electronBinaryPath) {
  ...
  if (
    ...
    if (isDevelopment) {
      ...
      writeDevelopmentLauncherScript(targetBinaryPath, developmentRuntimeBinaryPath);
    }
  ...
  if (isDevelopment) {
    NodeFS.renameSync(targetBinaryPath, developmentRuntimeBinaryPath);
    writeDevelopmentLauncherScript(targetBinaryPath, developmentRuntimeBinaryPath);
  }

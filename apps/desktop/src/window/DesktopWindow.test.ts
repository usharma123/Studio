...
function makeFakeBrowserWindow() {
  ...
  const webContents = {
    copyImageAt: vi.fn(),
    getURL: vi.fn(() => "codex-studio-dev://app/"),
...
describe("DesktopWindow", () => {
  it("builds a chrome-free assistant URL for the exact release thread", () => {
    assert.equal(
      DesktopWindow.getQaAssistantWindowUrl("codex-studio-dev://app/", {
        environmentId: "local" as never,
        threadId: "release/42" as never,
      }),
      "codex-studio-dev://app/?qaAssistant=detached#/local/release%2F42",
    );
  });

  it.effect("keeps one detached assistant window and reuses it across releases", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        const first = {
          environmentId: "local" as never,
          threadId: "release-1" as never,
          projectTitle: "Repro",
          releaseTitle: "Release 1",
        };
        yield* desktopWindow.detachQaAssistant(first);
        yield* desktopWindow.detachQaAssistant(first);
        yield* desktopWindow.detachQaAssistant({
          ...first,
          threadId: "release-2" as never,
          releaseTitle: "Release 2",
        });

        assert.equal(yield* Ref.get(createCount), 1);
        assert.deepEqual(fakeWindow.loadURL.mock.calls, [
          ["codex-studio-dev://app/?qaAssistant=detached#/local/release-1"],
          ["codex-studio-dev://app/?qaAssistant=detached#/local/release-2"],
        ]);
        assert.isUndefined(createdWindowOptions[0]?.webPreferences?.partition);

        yield* desktopWindow.closeQaAssistant;
  ...
  it("recognizes only same-origin renderer navigations", () => {
    assert.isTrue(
      DesktopWindow.isSameOriginRendererNavigation({
        applicationUrl: "codex-studio://app/",
        navigationUrl: "codex-studio://app/settings/connections",
      }),
    );
    assert.isFalse(
      DesktopWindow.isSameOriginRendererNavigation({
  ...
  it.effect("does not open a development window until the backend is ready", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        assert.deepEqual(fakeWindow.loadURL.mock.calls[0], ["codex-studio-dev://app/"]);
  ...
  it.effect("recovers when the development renderer is temporarily unreachable", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        didFailLoad({}, -9, "ERR_UNEXPECTED", "codex-studio-dev://app/", true);
  ...
  it("retries only transient failures for the development renderer", () => {
    assert.isTrue(
      DesktopWindow.isRetryableDevelopmentRendererLoadFailure({
        applicationUrl: "codex-studio-dev://app/",
        errorCode: -102,
        isMainFrame: true,
        validatedUrl: "codex-studio-dev://app/",
      }),
    );
    assert.isFalse(
      DesktopWindow.isRetryableDevelopmentRendererLoadFailure({

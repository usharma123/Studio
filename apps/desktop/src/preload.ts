...
contextBridge.exposeInMainWorld("desktopBridge", {
  ...
  getWindowChromeState: () => {
    const result = ipcRenderer.sendSync(IpcChannels.GET_WINDOW_CHROME_STATE_CHANNEL);
    return result as ReturnType<NonNullable<DesktopBridge["getWindowChromeState"]>>;
  },
  onWindowChromeStateChange: (listener) => {
    const wrappedListener = (_event: Electron.IpcRendererEvent, state: unknown) => {
      if (typeof state !== "object" || state === null) return;
      listener(state as Parameters<typeof listener>[0]);
    };

    ipcRenderer.on(IpcChannels.WINDOW_CHROME_STATE_CHANNEL, wrappedListener);
    return () => {
      ipcRenderer.removeListener(IpcChannels.WINDOW_CHROME_STATE_CHANNEL, wrappedListener);
    };
  },
  detachQaAssistant: (input) => ipcRenderer.invoke(IpcChannels.QA_ASSISTANT_DETACH_CHANNEL, input),
  closeQaAssistant: () => ipcRenderer.invoke(IpcChannels.QA_ASSISTANT_CLOSE_CHANNEL),
  onQaAssistantReattached: (listener) => {
    ...
    ipcRenderer.on(IpcChannels.QA_ASSISTANT_REATTACHED_CHANNEL, wrappedListener);
    return () => {
      ipcRenderer.removeListener(IpcChannels.QA_ASSISTANT_REATTACHED_CHANNEL, wrappedListener);
    };
  },

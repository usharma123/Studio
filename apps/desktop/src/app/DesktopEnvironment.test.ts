...
const defaultInput = {
  ...
  processArch: "arm64",
  appVersion: "0.0.22",
  appPath: "/Applications/Studio.app/Contents/Resources/app.asar",
  isPackaged: false,
...
describe("DesktopEnvironment", () => {
  it.effect("derives state paths and development identity inside Effect", () =>
    Effect.gen(function* () {
      ...
      assert.equal(environment.isPackaged, false);
      ...
      assert.equal(environment.appUserModelId, "com.codexstudio.desktop.dev");
      assert.equal(environment.branding.baseName, "Studio");
      assert.equal(environment.branding.stageLabel, "Dev");
      assert.equal(environment.branding.displayName, "Studio");
      assert.equal(environment.displayName, "Studio");
      assert.equal(environment.userDataDirName, "codex-studio-dev");
      assert.equal(environment.legacyUserDataDirName, "Codex Studio (Dev)");
      assert.equal(environment.linuxWmClass, "codex-studio-dev");
      assert.deepEqual(
  ...
  it.effect("derives production state paths under userdata", () =>
    Effect.gen(function* () {
      ...
      assert.equal(environment.branding.displayName, "Studio");
      assert.equal(environment.displayName, "Studio");
      assert.equal(environment.userDataDirName, "codex-studio");
      assert.equal(environment.legacyUserDataDirName, "Codex Studio (Alpha)");
    }),
  );
  ...
  it.effect("isolates the default backend state from upstream T3 Code", () =>
    Effect.gen(function* () {
      ...
      assert.equal(environment.baseDir, "/Users/alice/.codex-studio");
      assert.equal(environment.stateDir, "/Users/alice/.codex-studio/userdata");
    }),
  );
  ...
  it.effect("uses a configured app user model id override", () =>
    Effect.gen(function* () {
      const environment = yield* makeEnvironment(
        {},
        {
          T3CODE_DESKTOP_APP_USER_MODEL_ID: " com.codexstudio.desktop.dev.local ",
          VITE_DEV_SERVER_URL: "http://localhost:5173",
        },
      );

      assert.equal(environment.appUserModelId, "com.codexstudio.desktop.dev.local");
    }),
  );

  it.effect("isolates desktop user data by trusted development profile", () =>
    Effect.gen(function* () {
      const environment = yield* makeEnvironment(
        {},
        {
      ...
      assert.deepEqual(environment.developmentProfile, Option.some("qa:approver"));
      assert.equal(environment.branding.developmentProfile, "qa:approver");
      assert.equal(environment.userDataDirName, "codex-studio-dev-qa-approver");
    }),
  );

  it.effect("rejects unknown development profiles", () =>
    Effect.gen(function* () {
      const error = yield* makeEnvironment(
        {},
        {
          VITE_DEV_SERVER_URL: "http://localhost:5173",
          T3CODE_DEV_PROFILE: "qa:admin",
        },
      ).pipe(Effect.flip);

      assert.include(String(error), "T3CODE_DEV_PROFILE");
    }),
  );

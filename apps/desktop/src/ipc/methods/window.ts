import {
  ...
  DesktopQaAssistantWindowInputSchema,
  DesktopThemeSchema,
...
export const getWindowChromeState = DesktopIpc.makeSyncIpcMethod({
  ...
  handler: Effect.fn("desktop.ipc.window.getWindowChromeState")(function* () {
    ...
    return {
      isMaximized: window.value.isMaximized(),
      isFullScreen: window.value.isFullScreen() || window.value.isSimpleFullScreen(),
    };
  }),
});

export const detachQaAssistant = DesktopIpc.makeIpcMethod({
  channel: IpcChannels.QA_ASSISTANT_DETACH_CHANNEL,
  payload: DesktopQaAssistantWindowInputSchema,
  result: Schema.Void,
  handler: Effect.fn("desktop.ipc.window.detachQaAssistant")(function* (input) {
    const desktopWindow = yield* DesktopWindow.DesktopWindow;
    yield* desktopWindow.detachQaAssistant(input);
  }),
});

export const closeQaAssistant = DesktopIpc.makeIpcMethod({
  channel: IpcChannels.QA_ASSISTANT_CLOSE_CHANNEL,
  payload: Schema.Void,
  result: Schema.Void,
  handler: Effect.fn("desktop.ipc.window.closeQaAssistant")(function* () {
    const desktopWindow = yield* DesktopWindow.DesktopWindow;

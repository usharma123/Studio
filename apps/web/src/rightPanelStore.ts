  ...
  persist(
    (set) => ({
      ...
      reconcileBrowserSurfaces: (ref, tabIds) =>
        set((state) => {
          const byThreadKey = updateThread(state.byThreadKey, scopedThreadKey(ref), (current) => {
            ...
            const added = tabIds.flatMap((tabId) =>
              knownIds.has(`browser:${tabId}`) ? [] : [browserSurface(tabId)],
            );

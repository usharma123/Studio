...
describe("rightPanelStore", () => {
  ...
  it("keeps the QA dashboard as one persistent singleton surface", () => {
    useRightPanelStore.getState().open(refA, "qa");
    useRightPanelStore.getState().open(refA, "qa");

    expect(selectThreadRightPanelState(useRightPanelStore.getState().byThreadKey, refA)).toEqual({
      isOpen: true,
      activeSurfaceId: "qa",
      surfaces: [{ id: "qa", kind: "qa" }],
    });
  });

  it("preserves a saved QA dashboard through persistence migration", () => {
    expect(
      migratePersistedRightPanelState({
        byThreadKey: {
          "env-1:thread-A": {
  ...
  it("does not publish a store update when browser surfaces are already reconciled", () => {
    useRightPanelStore.getState().openBrowser(refA, "tab-a");

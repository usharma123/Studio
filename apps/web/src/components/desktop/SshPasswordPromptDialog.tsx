...
async function resolveSshPasswordPrompt(
  requestId: string,
  password: string | null,
): Promise<string | null> {
  try {
    await window.desktopBridge?.resolveSshPasswordPrompt(requestId, password);
    return null;
  } catch (error) {
    return getPromptErrorMessage(error);
  }
}
...
function ActiveSshPasswordPrompt({
  ...
  const respond = async (nextPassword: string | null) => {
    ...
    const resolutionError = await resolveSshPasswordPrompt(requestId, nextPassword);
    if (resolutionError === null || nextPassword === null) {
      onRemove(requestId);
    } else {
      setResponseError(resolutionError);
    }

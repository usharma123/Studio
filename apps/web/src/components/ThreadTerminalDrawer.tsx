...
import { createAnimationFrameBatcher, createTrailingBatcher } from "~/lib/eventBatcher";
...
} from "./terminalSelectionAction.logic";
import { useAtomCommand } from "../state/use-atom-command";
const MIN_DRAWER_HEIGHT = 180;
const MAX_DRAWER_HEIGHT_RATIO = 0.75;
const WINDOW_RESIZE_SETTLE_MS = 100;
...
async function requestAddTerminalSelection(
  api: NonNullable<ReturnType<typeof readLocalApi>>,
  position: {
    readonly x: number;
    readonly y: number;
  },
...
function useTerminalViewportContent({
  threadRef,
  threadId,
  terminalId,
  terminalLabel,
  ...
  const runtimeEnvKey = runtimeEnvSignature(runtimeEnv);
  ...
  useEffect(() => {
    ...
    const showSelectionAction = async () => {
      ...
      const shouldAdd = await requestAddTerminalSelection(localApi, nextAction.position);
      selectionActionOpenRef.current = false;
      if (requestId !== selectionActionRequestIdRef.current || !shouldAdd) {
        return;
      }
    ...
    const terminalLinksDisposable = terminal.registerLinkProvider({
      provideLinks: (bufferLineNumber, callback) => {
        ...
        const links: Array<{
          match: ReturnType<typeof extractTerminalLinks>[number];
          range: ReturnType<typeof resolveWrappedTerminalLinkRange>;
        }> = [];
...
function TerminalViewport(props: TerminalViewportProps) {
  return <>{useTerminalViewportContent(props)}</>;
}
...
function useThreadTerminalDrawerContent({
  mode = "drawer",
  threadRef,
  threadId,
  cwd,
  ...
  const setDrawerHeight = (update: SetStateAction<number>) => {
    setDrawerHeightState((current) => {
      const currentHeight = current.threadId === threadId ? current.height : controlledDrawerHeight;
  ...
  const terminalResizeEpoch = resizeEpoch * 2 + Number(visible);
  ...
  const resolveTerminalLaunchLocation = (terminalId: string): TerminalLaunchLocation => {
    return (
  ...
  const syncHeightFromEffect = useEffectEvent(syncHeight);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const resizeBatcher = createAnimationFrameBatcher<void>(() => {
      ...
      if (!resizeStateRef.current) {
        syncHeightFromEffect(clampedHeight);
      }
    });
    const resizeSettler = createTrailingBatcher<void>(
      () => setResizeEpoch((value) => value + 1),
      WINDOW_RESIZE_SETTLE_MS,
    );
    const onWindowResize = () => {
      resizeBatcher.schedule(undefined);
      resizeSettler.schedule(undefined);
    };
  ...
  useEffect(() => {
    return () => {
      syncHeightFromEffect(drawerHeightRef.current);
    };
  }, []);
  if (normalizedTerminalIds.length === 0) {
    return (
                ...
                {visibleTerminalIds.map((terminalId) => {
                  ...
                  return (
                    <div
                      key={terminalId}
                      role="presentation"
                      className={`min-h-0 min-w-0 ${splitDirection === "vertical" ? "border-t first:border-t-0" : "border-l first:border-l-0"} ${terminalId === resolvedActiveTerminalId ? "border-border" : "border-border/70"}`}
                      onMouseDown={() => {
                    ...
                    >
                      <div className="h-full p-1">
                        <TerminalViewport
                          ...
                          resizeEpoch={terminalResizeEpoch}
            ...
            <aside className="flex w-36 min-w-36 flex-col border border-border/70 bg-muted/10">
              <div className="flex h-[22px] items-stretch justify-end border-b border-border/70">
                <div className="inline-flex h-full items-stretch">
                  ...
                  <TerminalActionButton
                    className={`inline-flex h-full items-center border-l border-border/70 px-1 text-foreground/90 transition-colors ${hasReachedSplitLimit ? "cursor-not-allowed opacity-45 hover:bg-transparent" : "hover:bg-accent/70"}`}
                    onClick={onSplitTerminalVerticalAction}
                    label={splitTerminalVerticalActionLabel}
                  >
                    <SquareSplitVertical className="size-3.25" />
...
export default function ThreadTerminalDrawer(props: ThreadTerminalDrawerProps) {
  return <>{useThreadTerminalDrawerContent(props)}</>;
}

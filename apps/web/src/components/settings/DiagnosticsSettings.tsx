...
import { DiagnosticsSettingsPanelView } from "./DiagnosticsSettingsPanelView";
...
function useDiagnosticsSettingsPanelController() {
  ...
  const { data, error, isPending, refresh } = useEnvironmentQuery(
    environmentId === null
      ? null
      : serverEnvironment.traceDiagnostics({
          environmentId,
          input: {},
        }),
  );
  ...
  const signalProcess = (pid: number, signal: ServerProcessSignal) => {
    if (
      signal === "SIGKILL" &&
...
export type DiagnosticsSettingsPanelController = ReturnType<
  typeof useDiagnosticsSettingsPanelController
>;
export function DiagnosticsSettingsPanel() {
  const controller = useDiagnosticsSettingsPanelController();
  return <DiagnosticsSettingsPanelView controller={controller} />;
}

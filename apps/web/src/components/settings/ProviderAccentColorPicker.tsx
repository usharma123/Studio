...
function ProviderCustomColorPanel(props: {
  ...
  const commitHsv = (nextHsv: typeof hsv) => {
    setHsv(nextHsv);
    onCommit(hsvToHex(nextHsv.h, nextHsv.s, nextHsv.v));
  };

  const updateFromPlane = (event: PointerEvent<HTMLDivElement>) => {
  ...
  return (
    <div className="w-56 bg-popover">
      <div
        ...
        onPointerDown={capturePointer(updateFromPlane)}
        onPointerMove={(event) => {
      ...
      <div className="grid gap-3 p-3">
        <div
          ...
          onPointerDown={capturePointer(updateFromHue)}
          onPointerMove={(event) => {
            if (event.currentTarget.hasPointerCapture(event.pointerId)) {
              updateFromHue(event);
            }

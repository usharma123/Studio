...
function useFileLineReveal(
  ...
  return ((fileContainer, instance, phase) => {
...
function EditableFileSurface({
  ...
  const setSelectedRange = (range: SelectedLineRange | null) => {
  ...
  const removeAnnotationEntry = (entryId: string) => {
  ...
  const submitAnnotationEntry = (entryId: string, text: string) => {
    setSelectedRange(null);
  ...
  const beginComment = (range: SelectedLineRange) => {
  ...
  useEffect(() => {
    ...
    return installFileEditorDismissal({
      ...
      onDismiss: () => {
        setSelectionOverride({ revealRequestId, range: null });
      },
    });
  }, [editor, hasOpenCommentForm, revealRequestId]);
  ...
  const handlePostRender = ((fileContainer, instance, phase) => {
    ...
    selectionFrameRef.current = requestAnimationFrame(() => {
      ...
      instance.setSelectedLines(selectedRange, {
        notify: false,
      });
    });
  }) as FilePostRender;
...
export default function FilePreviewPanel({
  ...
  const breadcrumbs = relativePath ? fileBreadcrumbs(projectName, relativePath) : [];
  const onFilePostRender = useFileLineReveal(relativePath, revealLine, revealRequestId);
  useEffect(() => {
    const currentCrumb = breadcrumbRef.current?.querySelector<HTMLElement>(
      "[data-current-file-crumb='true']",
    );
    currentCrumb?.scrollIntoView({
      block: "nearest",
      inline: "end",
    });
  }, [relativePath]);

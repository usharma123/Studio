...
import { createTrailingBatcher } from "~/lib/eventBatcher";
    ...
    <div
      ...
      style={
        isInline && !props.maximized
          ? { width: `${width}px`, maxWidth: `${PREVIEW_PANEL_MAX_WIDTH_FRACTION * 100}vw` }
          : undefined
      }
...
function useViewportClampedMaxWidth(): number {
  ...
  useEffect(() => {
    ...
    const resizeSettler = createTrailingBatcher<number>(setVw, WINDOW_RESIZE_SETTLE_MS);
    const onResize = () => {
      resizeSettler.schedule(window.innerWidth);
    };

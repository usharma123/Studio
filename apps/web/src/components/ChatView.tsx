...
import { NewTaskWelcome } from "./chat/NewTaskWelcome";
...
import { ChevronDownIcon, MessageCircleIcon, TriangleAlertIcon, WifiOffIcon } from "lucide-react";
...
function copyRightPanelFilePath(relativePath: string): void {
...
function useMountedTerminalThreadKeys(input: {
  readonly openThreadIds: readonly string[];
  readonly activeThreadId: string | null;
  readonly activeThreadTerminalOpen: boolean;
}): string[] {
  const [snapshot, setSnapshot] = useState(() => ({
    ...input,
    threadIds: reconcileMountedTerminalThreadIds({
      currentThreadIds: [],
      ...input,
  ...
  if (
    !haveSameIds(snapshot.openThreadIds, input.openThreadIds) ||
    snapshot.activeThreadId !== input.activeThreadId ||
    snapshot.activeThreadTerminalOpen !== input.activeThreadTerminalOpen
  ) {
    const threadIds = reconcileMountedTerminalThreadIds({
      currentThreadIds: snapshot.threadIds,
      ...input,
      maxHiddenThreadCount: MAX_HIDDEN_MOUNTED_TERMINAL_THREADS,
    });
    setSnapshot({ ...input, threadIds });
    return threadIds;
  }
...
function useLocalDispatchState(input: {
  ...
  const serverAcknowledgedLocalDispatch = (() =>
    hasServerAcknowledgedLocalDispatch({
      localDispatch,
      phase: input.phase,
...
const PersistentThreadTerminalDrawer = function PersistentThreadTerminalDrawer({
  threadRef,
  threadId,
  visible,
  launchContext,
  ...
  const panelTerminalIds = (() =>
    new Set(
      panelSurfaces.flatMap((surface) => (surface.kind === "terminal" ? surface.terminalIds : [])),
    ))();
  const drawerTerminalSessions = (() =>
  ...
  const terminalLaunchLocationsById = (() => {
    const next = new Map<
      string,
      {
        readonly cwd: string;
  ...
  const serverOrderedTerminalIds = (() =>
  ...
  useEffect(() => {
    if (haveSameIds(serverOrderedTerminalIds, terminalUiState.terminalIds)) {
      return;
    }
    if (
  ...
  const handleAddTerminalContext = (selection: TerminalContextSelection) => {
    if (!visible) {
      return;
    }
...
const PersistentThreadTerminalPanel = function PersistentThreadTerminalPanel({
  threadRef,
  surface,
  launchContext,
  focusRequestId,
  ...
  const terminalSessionSummaryById = new Map(
    knownTerminalSessions.map(
      (session) => [session.target.terminalId, session.state.summary] as const,
    ),
  );
  const threadWorktreePath = serverThread?.worktreePath ?? draftThread?.worktreePath ?? null;
  const activeSummary = terminalSessionSummaryById.get(surface.activeTerminalId) ?? null;
  const worktreePath =
  ...
  const terminalLabelsById = (() => {
    ...
    for (const terminalId of surface.terminalIds) {
      const summary = terminalSessionSummaryById.get(terminalId) ?? null;
...
function useChatViewContent(props: ChatViewProps) {
  const {
    environmentId,
    threadId,
    routeKind,
  ...
  const enterpriseMode = useEnterpriseModeStore((state) => state.mode);
  const detachedQaAssistant = isCurrentWindowDetachedQaAssistant();
  const isQaMode = enterpriseMode === "qa" || detachedQaAssistant;
  ...
  const shouldUseRightPanelSheet = shouldUsePlanSidebarSheet && !isQaMode;
  ...
  const serverThreadKeys = (() => serverThreadRefs.map(scopedThreadKey))();
  ...
  const activeThreadKnownSessions = (() => {
  ...
  const mountedTerminalThreadKeys = useMountedTerminalThreadKeys({
    openThreadIds: existingOpenTerminalThreadKeys,
    activeThreadId: activeThreadKey,
    activeThreadTerminalOpen: Boolean(activeThreadKey && terminalUiState.terminalOpen),
  });
  const mountedTerminalThreadRefs = (() =>
  ...
  const qaAssistantPresentation = useQaAssistantStore((state) =>
    selectQaAssistantPresentation(state.byThreadKey, activeThreadRef),
  );
  const effectiveQaAssistantPresentation = detachedQaAssistant ? "open" : qaAssistantPresentation;
  const openQaAssistant = useQaAssistantStore((state) => state.open);
  const minimizeQaAssistant = useQaAssistantStore((state) => state.minimize);
  const closeQaAssistant = useQaAssistantStore((state) => state.close);
  const markQaAssistantDetached = useQaAssistantStore((state) => state.markDetached);
  useEffect(() => {
    return window.desktopBridge?.onQaAssistantReattached?.((input) => {
      useQaAssistantStore.getState().minimize(scopeThreadRef(input.environmentId, input.threadId));
    });
  }, []);
  ...
  const rightPanelMaximized =
    ...
    !detachedQaAssistant &&
    (isQaMode || maximizedRightPanelThreadKey === routeThreadKey);
  ...
  const detachQaAssistant = async () => {
    if (!activeThreadRef || !activeThread || !window.desktopBridge?.detachQaAssistant) return;
    markQaAssistantDetached(activeThreadRef);
    await window.desktopBridge
      .detachQaAssistant({
        environmentId: activeThreadRef.environmentId,
        threadId: activeThreadRef.threadId,
        projectTitle: activeProject?.title ?? "QA project",
        releaseTitle: activeThread.title,
      })
      .catch((error: unknown) => {
        openQaAssistant(activeThreadRef);
        toastManager.add({
          type: "error",
          title: "Could not detach the release assistant",
          description:
            error instanceof Error ? error.message : "The assistant window did not open.",
        });
      });
  };
  ...
  const handleFilePendingChange = (relativePath: string, pending: boolean) => {
  ...
  const configuredPreviewUrls = (() => getConfiguredPreviewUrls(activeProject?.scripts))();
  useEffect(() => {
  ...
  const handleReconnectActiveEnvironment = async (environmentId: EnvironmentId) => {
  ...
  const workLogEntries = (() => deriveWorkLogEntries(threadActivities))();
  ...
  const activePendingDraftAnswers = (() =>
  ...
  const activePendingProgress = (() =>
    activePendingUserInput
      ...
      : null)();
  const activePendingResolvedAnswers = (() =>
    activePendingUserInput
  ...
  const clearAttachmentPreviewHandoffsOnUnmount = useEffectEvent(clearAttachmentPreviewHandoffs);
  useEffect(() => {
    return () => {
      clearAttachmentPreviewHandoffsOnUnmount();
  ...
  const handoffAttachmentPreviewsFromEffect = useEffectEvent(handoffAttachmentPreviews);
  const serverMessages = activeThread?.messages;
  const serverAttachmentIds = (() => {
  ...
  const serverAttachmentResources = (() =>
    serverAttachmentIds.map((attachmentId) => ({
      _tag: "attachment" as const,
      attachmentId,
    })))();
  const serverAttachmentUrls = useAssetUrls(environmentId, serverAttachmentResources);
  const serverAttachmentUrlById = (() =>
    new Map(
  ...
  const displayServerMessages: ChatMessage[] = (() => {
  ...
  const clearAttachmentPreviewHandoffFromEffect = useEffectEvent(clearAttachmentPreviewHandoff);
  useEffect(() => {
    ...
    for (const message of displayServerMessages) {
      if (message.role === "user") {
        userMessagesById.set(String(message.id), message);
      }
    }
    for (const [messageId, handoffPreviewUrls] of Object.entries(
      ...
      void preloadServerPreviews
        .then(() => {
          ...
          clearAttachmentPreviewHandoffFromEffect(messageId as MessageId, handoffPreviewUrls);
        })
        .catch(() => {
          if (!cancelled) {
            delete attachmentPreviewPromotionInFlightByMessageIdRef.current[messageId];
  ...
  const splitTerminalVerticalShortcutLabel = (() =>
    shortcutLabelForCommand(keybindings, "terminal.splitVertical", terminalShortcutLabelOptions))();
  const newTerminalShortcutLabel = (() =>
    shortcutLabelForCommand(keybindings, "terminal.new", terminalShortcutLabelOptions))();
  const closeTerminalShortcutLabel = (() =>
  ...
  const onEnvironmentChange = (nextEnvironmentId: EnvironmentId) => {
  ...
  const selectNewTaskSuggestion = (nextPrompt: string) => {
  ...
  const splitTerminal = (direction: "horizontal" | "vertical" = "horizontal") => {
  ...
  const deleteProjectScript = async (
    scriptId: string,
  ): Promise<AtomCommandResult<void, unknown>> => {
    if (!activeProject) {
      return AsyncResult.success(undefined);
  ...
  const dismissPlanSidebarForCurrentTurn = () => {
  ...
  const splitPanelTerminal = (direction: "horizontal" | "vertical" = "horizontal") => {
    if (
      !activeThreadRef ||
      !activeThreadId ||
      !activeProject ||
  ...
  const splitPanelTerminalVertical = () => {
  ...
  const closePanelTerminal = (terminalId: string) => {
    if (!activeThreadRef || activeRightPanelSurface?.kind !== "terminal") return;
  ...
  const cleanupRightPanelSurfaces = (surfaces: readonly RightPanelSurface[]) => {
    if (!activeThreadRef) return;
  ...
  const closeAllRightPanelSurfaces = () => {
    if (!activeThreadRef) return;
    cleanupRightPanelSurfaces(rightPanelState.surfaces);
    useRightPanelStore.getState().closeAllSurfaces(activeThreadRef);
  };
  const togglePreviewPanelFromEffect = useEffectEvent(togglePreviewPanel);
  useEffect(
    () =>
      subscribePreviewAction((action) => {
        if (action === "toggle-panel") togglePreviewPanelFromEffect();
      }),
    [],
  );
  ...
  const [showScrollDebouncer] = useState(
  ...
  const cancelTimelineLiveFollowForUserNavigation = () => {
  ...
  const getActiveTimelineTurnMetrics = (list?: LegendListRef | null) => {
  ...
  const timelineRealContentOverflowsViewport = (list?: LegendListRef | null) => {
    const resolvedList = list ?? legendListRef.current;
    const state = resolvedList?.getState();
  ...
  const getActiveTimelineTurnMetricsFromEffect = useEffectEvent(getActiveTimelineTurnMetrics);
  const timelineRealContentOverflowsViewportFromEffect = useEffectEvent(
  ...
  const onTimelineAnchorReady = (messageId: MessageId, anchorIndex: number) => {
  ...
  const onTimelineAnchorSizeChanged = (messageId: MessageId) => {
    ...
    const frame = requestAnimationFrame(() => {
      secondFrame = requestAnimationFrame(() => {
        ...
        if (timelineScrollModeRef.current === "anchoring-new-turn") {
          const metrics = getActiveTimelineTurnMetricsFromEffect(list);
          if (!metrics) {
            return;
          }
        ...
        if (!timelineRealContentOverflowsViewportFromEffect(list)) {
          return;
        }
  ...
  }, [activeThread?.id, activeThreadRef, showScrollDebouncer]);
  ...
  useEffect(() => {
    const stateSyncTimer = window.setTimeout(() => setIsRevertingCheckpoint(false), 0);
    return () => window.clearTimeout(stateSyncTimer);
  }, [activeThread?.id]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    for (const removedMessage of removedMessages) {
      ...
      if (previewUrls.length > 0) {
        handoffAttachmentPreviewsFromEffect(removedMessage.id, previewUrls);
        continue;
      }
  ...
  const handleWindowKeyDown = useEffectEvent((event: globalThis.KeyboardEvent) => {
  ...
  const onSubmitPlanFollowUp = async ({
    ...
    preserveRightPanel = false,
  }: {
    text: string;
    interactionMode: "default" | "plan";
    preserveRightPanel?: boolean;
  }) => {
    ...
    if (failure === null) {
      ...
      const startResult = await startThreadTurn({
        ...
        input: {
          ...
          runtimeMode,
          interactionMode: nextInteractionMode,
          ...(nextInteractionMode === "default" && activeProposedPlan && !preserveRightPanel
            ? {
                sourceProposedPlan: {
                  threadId: activeThread.id,
    ...
    if (failure === null) {
      ...
      if (nextInteractionMode === "default" && autoOpenPlanSidebar && !preserveRightPanel) {
        planSidebarDismissedForTurnRef.current = null;
        if (activeThreadRef) {
          useRightPanelStore.getState().open(activeThreadRef, "plan");
        }
  ...
  const getModelDisabledReason = (instanceId: ProviderInstanceId, model: string): string | null => {
    if (!activeThread) {
      return null;
    }
  ...
  const onProviderModelSelect = (instanceId: ProviderInstanceId, model: string) => {
    if (!activeThread) return;
  ...
  const panelToggleControls = (
    <PanelLayoutControls
      ...
      showTerminal={!isQaMode}
    />
  );
  const panelLayoutControls = isQaMode ? null : (
    ...
    <div className="relative flex min-h-0 min-w-0 flex-1 overflow-hidden bg-background">
      ...
      <div
        className={cn(
          ...
          isQaMode &&
            !detachedQaAssistant &&
            effectiveQaAssistantPresentation === "open" &&
            "fixed right-4 bottom-4 z-[60] h-[min(760px,calc(100dvh-5rem))] w-[min(620px,calc(100vw-2rem))] overflow-hidden rounded-[24px] border border-border/80 bg-background shadow-2xl",
          isQaMode &&
            !detachedQaAssistant &&
            effectiveQaAssistantPresentation === "minimized" &&
            "fixed right-4 bottom-4 z-[60] h-12 w-[min(420px,calc(100vw-2rem))] overflow-hidden rounded-2xl border border-border/80 bg-background shadow-2xl",
          isQaMode &&
            !detachedQaAssistant &&
            (effectiveQaAssistantPresentation === "closed" ||
              effectiveQaAssistantPresentation === "detached") &&
            "hidden",
        )}
        data-chat-column-maximized-away={rightPanelMaximized ? "true" : "false"}
      >
        {/* Top bar */}
        {isQaMode && activeThreadRef ? (
          <QaAssistantHeader
            presentation={effectiveQaAssistantPresentation}
            releaseTitle={activeThread.title}
            projectTitle={activeProject?.title ?? "QA project"}
            detached={detachedQaAssistant}
            canDetach={Boolean(window.desktopBridge?.detachQaAssistant)}
            onOpen={() => openQaAssistant(activeThreadRef)}
            onMinimize={() => {
              if (detachedQaAssistant) {
                void window.desktopBridge?.closeQaAssistant?.();
              } else {
                minimizeQaAssistant(activeThreadRef);
              }
            }}
            onClose={() => {
              if (detachedQaAssistant) {
                ...
                closeQaAssistant(activeThreadRef);
              }
            }}
            onDetach={() => void detachQaAssistant()}
          />
        ) : (
          <header
            data-chat-header
        ...
        <div
          className={cn(
            ...
            isQaMode && effectiveQaAssistantPresentation !== "open" && "hidden",
          )}
        >
          {/* Error banner */}
          ...
          <div className="flex min-h-0 min-w-0 flex-1">
            ...
            <div className="relative flex min-h-0 min-w-0 flex-1 flex-col">
                ...
                <MessagesTimeline
                  ...
                  emptyState={
                    <NewTaskWelcome
                      ...
                      onSelectSuggestion={selectNewTaskSuggestion}
                      mode={isQaMode ? "qa" : "developer"}
                    />
                  }
                />
              ...
              >
                ...
                >
                  <div className="relative mx-auto h-full w-full max-w-4xl overflow-clip rounded-t-[20px]">
              ...
              {pullRequestDialogState && !isQaMode ? (
      ...
      effectiveQaAssistantPresentation === "closed" ? (
        <button
          type="button"
          className="fixed right-4 bottom-4 z-50 inline-flex h-11 items-center gap-2 rounded-full border border-border/80 bg-card px-4 text-sm font-medium shadow-xl transition hover:bg-accent"
          onClick={() => openQaAssistant(activeThreadRef)}
          aria-label="Open release assistant"
        >
      ...
      {!detachedQaAssistant && !shouldUseRightPanelSheet && rightPanelOpen && activeThreadRef ? (
        <RightPanelTabs
          mode="inline"
          maximized={rightPanelMaximized}
...
function ChatViewContent(props: ChatViewProps) {
  return useChatViewContent(props);
}

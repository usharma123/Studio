...
function useDiffPanelContent({ mode = "inline", composerDraftTarget }: DiffPanelProps) {
  ...
  const renderablePatch = getRenderablePatch(selectedPatch, `diff-panel:${resolvedTheme}`, {
  ...
  const codeViewFiles = renderableFiles.map((fileDiff) => {
  ...
  const openDiffFile = (filePath: string) => {
...
export default function DiffPanel(props: DiffPanelProps) {
  return <>{useDiffPanelContent(props)}</>;
}

...
export function ProviderUpdateEnvironmentRows({
  ...
  const groupByEnvironment = new Map(groups.map((group) => [group.environmentId, group] as const));
  ...
  >(() => new Map());
  const clearPending = (environmentId: EnvironmentId) => {
  ...
  const handleUpdate = async (environmentId: EnvironmentId) => {
  ...
  const rows = groups.reduce<
    Array<{
      ...
      status: ReturnType<typeof resolveEnvironmentUpdateRowStatus>;
    }>
  >((visibleRows, group) => {
    const status = resolveEnvironmentUpdateRowStatus({
      group,
    ...
    if (group.candidates.length > 0 || status.kind !== "idle") visibleRows.push({ group, status });
    return visibleRows;
  }, []);
  if (rows.length === 0) {
    return null;

...
describe("haveSameIds", () => {
  it("treats freshly derived identity arrays with the same values as equal", () => {
    expect(haveSameIds(["thread-a", "thread-b"], ["thread-b", "thread-a"])).toBe(true);
    expect(haveSameIds(["thread-a"], ["thread-b"])).toBe(false);
  });
});

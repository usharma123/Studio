...
export function PairingRouteSurface({
  ...
  const submitCredential = async (nextCredential: string) => {
  ...
  const submitCredentialFromEffect = useEffectEvent(submitCredential);
  useEffect(() => {
    ...
    stripPairingTokenFromUrl();
    void submitCredentialFromEffect(autoPairToken);
  }, [autoPairToken]);
  return (
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-background px-4 py-10 text-foreground sm:px-6">
      <div className="pointer-events-none absolute inset-0 opacity-80">
...
export function HostedPairingRouteSurface() {
  ...
  const submitHostedPairingRequest = async () => {
    ...
    if (result._tag === "Success") {
      setStatus("paired");
      setMessage(`${hostedPairingRequest.label || "The environment"} is saved in this browser.`);
      return;
    }
  ...
  const submitHostedPairingRequestFromEffect = useEffectEvent(submitHostedPairingRequest);
  useEffect(() => {
    ...
    stripPairingTokenFromUrl();
    void submitHostedPairingRequestFromEffect();
  }, []);
  return (
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-background px-4 py-10 text-foreground sm:px-6">
      <div className="pointer-events-none absolute inset-0 opacity-80">
      ...
      <section className="relative w-full max-w-xl rounded-2xl border border-border/80 bg-card/90 p-6 shadow-2xl shadow-black/20 backdrop-blur-md sm:p-8">
        ...
        {hostedPairingRequest ? (
          <div className="mt-5 rounded-lg border border-border/70 bg-background/55 px-3 py-3 text-xs leading-relaxed text-muted-foreground">
            Host: <span className="font-mono text-foreground/80">{hostedPairingRequest.host}</span>
          </div>
        ) : null}

...
interface AddProjectFlowState {
  addProjectEnvironmentId: EnvironmentId | null;
  isPickingProjectFolder: boolean;
  addProjectCloneFlow: AddProjectCloneFlow | null;
  isRemoteProjectLookingUp: boolean;
  isRemoteProjectCloning: boolean;
}
...
export function CommandPalette({ children }: { children: ReactNode }) {
  ...
  const clearOpenIntent = () => dispatch({ _tag: "ClearOpenIntent" });
  ...
  const toggleOpenFromShortcut = useEffectEvent(() => dispatch({ _tag: "Toggle" }));
  useEffect(() => {
      ...
      <ComposerHandleContext value={composerHandleRef}>
        ...
        <CommandDialog open={state.open} onOpenChange={setOpen}>
          ...
          <CommandPaletteDialog
            ...
            openQaProjectCreation={() => setQaProjectCreationOpen(true)}
          />
        </CommandDialog>
...
function CommandPaletteDialog(props: {
  ...
  readonly openQaProjectCreation: () => void;
}) {
  if (!props.open) {
    return null;
  }
  return (
    <OpenCommandPaletteDialog
      ...
      openQaProjectCreation={props.openQaProjectCreation}
    />
  );
}
function useOpenCommandPaletteDialogContent(props: {
  readonly openIntent: CommandPaletteOpenIntent | null;
  readonly setOpen: (open: boolean) => void;
  readonly clearOpenIntent: () => void;
  readonly openQaProjectCreation: () => void;
  ...
  const [addProjectFlow, updateAddProjectFlow] = useReducer(
    (state: AddProjectFlowState, patch: Partial<AddProjectFlowState>) => ({ ...state, ...patch }),
    {
      addProjectEnvironmentId: null,
      isPickingProjectFolder: false,
      addProjectCloneFlow: null,
      isRemoteProjectLookingUp: false,
      isRemoteProjectCloning: false,
    },
  );
  const {
  ...
  const addProjectEnvironmentOptions = (() => {
  ...
  const wslAddProjectEnvironmentOption = (() =>
  ...
  const getAddProjectInitialQueryForEnvironment = (environmentId: EnvironmentId | null): string => {
  ...
  const { filteredEntries: filteredBrowseEntries, exactEntry: exactBrowseEntry } = (() =>
    filterBrowseEntries({ browseEntries, browseFilterQuery, highlightedItemValue }))();
  const openProjectFromSearch = (() => async (project: (typeof projects)[number]) => {
  ...
  function popView(): void {
    updateAddProjectFlow({ addProjectCloneFlow: null });
    if (viewStack.length <= 1) {
      updateAddProjectFlow({ addProjectEnvironmentId: null });
    }
  ...
  const startAddProjectBrowse = (environmentId: EnvironmentId): void => {
    updateAddProjectFlow({ addProjectEnvironmentId: environmentId, addProjectCloneFlow: null });
    pushPaletteView({
  ...
  const startAddProjectClone = (
    environmentId: EnvironmentId,
    source: AddProjectRemoteSource,
  ): void => {
    updateAddProjectFlow({
      addProjectEnvironmentId: environmentId,
      addProjectCloneFlow: { step: "repository", environmentId, source },
    });
    pushPaletteView({
  ...
  const buildAddProjectSourceGroups = (
    environmentId: EnvironmentId,
    readinessBySource: AddProjectRemoteSourceReadiness,
  ...
  const startAddProjectSourceSelection = (environmentId: EnvironmentId): void => {
    updateAddProjectFlow({ addProjectEnvironmentId: environmentId, addProjectCloneFlow: null });
    pushPaletteView({
      addonIcon: <FolderPlusIcon className={ADDON_ICON_CLASS} />,
  ...
  const addProjectEnvironmentGroups = (() => [
    {
      value: "environments",
      label: "Environments",
  ...
  const openAddProjectFromIntent = useEffectEvent(() => {
    clearOpenIntent();
    openAddProjectFlow();
  });
  useLayoutEffect(() => {
    ...
    const task = window.setTimeout(openAddProjectFromIntent, 0);
  ...
  const handleAddProject = async (rawCwd: string) => {
  ...
  async function submitAddProjectCloneFlow(destinationPathInput?: string): Promise<void> {
    ...
    if (addProjectCloneFlow.step === "repository") {
      ...
      updateAddProjectFlow({ isRemoteProjectLookingUp: true });
      ...
      updateAddProjectFlow({ isRemoteProjectLookingUp: false });
    ...
    updateAddProjectFlow({ isRemoteProjectCloning: true });
    ...
    updateAddProjectFlow({ isRemoteProjectCloning: false });
  ...
  const handleOpenProjectFromFileManager = async () => {
    ...
    updateAddProjectFlow({ isPickingProjectFolder: true });
    ...
    try {
      ...
      updateAddProjectFlow({ isPickingProjectFolder: false });
      return;
    }
    updateAddProjectFlow({ isPickingProjectFolder: false });
    if (!pickedPath) {
...
function OpenCommandPaletteDialog(props: Parameters<typeof useOpenCommandPaletteDialogContent>[0]) {
  return useOpenCommandPaletteDialogContent(props);
}

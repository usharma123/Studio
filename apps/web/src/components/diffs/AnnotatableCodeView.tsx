...
export function AnnotatableCodeView({
  ...
  const filesByKey = new Map(files.map((file) => [file.fileKey, file]));
  const items = files.map(({ fileDiff, filePath, fileKey, collapsed }) => {
    ...
    const annotations = draft?.fileKey === fileKey ? [...persisted, draft.annotation] : persisted;
    return {
      id: fileKey,
      type: "diff",
      fileDiff,
      ...
      collapsed,
      version: fnv1a32(
        `${collapsed ? "1" : "0"}:${annotations.flatMap((annotation) => annotation.metadata.entries.map((entry) => `${entry.id}:${entry.rangeLabel}:${entry.text}`)).join(":")}`,
      ),
    };
  }) as CodeViewDiffItem<DiffCommentAnnotationGroup>[];
  ...
  const beginComment = (range: SelectedLineRange | null, context: DiffSelectionContext) => {
    if (!range) return;

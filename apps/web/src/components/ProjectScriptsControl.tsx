...
interface ProjectScriptsControlState {
...
function useProjectScriptsControlContent({
  scripts,
  keybindings,
  ...
  const [state, updateState] = useReducer(
    (current: ProjectScriptsControlState, patch: Partial<ProjectScriptsControlState>) => ({
      ...current,
      ...patch,
    }),
    {
  ...
  const setEditingScriptId = (value: string | null) => updateState({ editingScriptId: value });
  const setDialogOpen = (value: boolean) => updateState({ dialogOpen: value });
  const setName = (value: string) => updateState({ name: value });
  const setCommand = (value: string) => updateState({ command: value });
  const setIcon = (value: ProjectScriptIcon) => updateState({ icon: value });
  const setIconPickerOpen = (value: boolean) => updateState({ iconPickerOpen: value });
  const setRunOnWorktreeCreate = (value: boolean) => updateState({ runOnWorktreeCreate: value });
  const setKeybinding = (value: string) => updateState({ keybinding: value });
  const setPreviewUrl = (value: string) => updateState({ previewUrl: value });
  const setAutoOpenPreview = (value: boolean) => updateState({ autoOpenPreview: value });
  const setValidationError = (value: string | null) => updateState({ validationError: value });
  const setDeleteConfirmOpen = (value: boolean) => updateState({ deleteConfirmOpen: value });
  const primaryScript = (() => {
        ...
        <DialogPopup>
          ...
          <DialogPanel>
            <form id={addScriptFormId} className="space-y-4" onSubmit={submitAddScript}>
              ...
              <label
                className={`flex items-center justify-between gap-3 rounded-md border border-border/70 px-3 py-2 text-sm ${previewUrl.trim().length === 0 ? "opacity-60" : ""}`}
              >
...
export default function ProjectScriptsControl(props: ProjectScriptsControlProps) {
  return useProjectScriptsControlContent(props);
}

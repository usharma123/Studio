...
function preventMiddleMouseDefault(event: ReactMouseEvent): void {
  ...
  const actions = [
    ...
    },
    {
      ...
      description: props.qaMode
        ? "Open a configured application environment."
...
export function RightPanelTabs(props: RightPanelTabsProps) {
  ...
  const handleTabContextMenu = async (event: ReactMouseEvent, surface: RightPanelSurface) => {
    event.preventDefault();
  ...
  const handleTabAuxClick = (event: ReactMouseEvent, surface: RightPanelSurface) => {
    if (event.button !== 1) return;
    event.preventDefault();
    event.stopPropagation();
    props.onCloseSurface(surface);
            ...
            {props.surfaces.length > 0 ? (
              <Menu>
                ...
                <MenuPopup align="start" side="bottom" sideOffset={6} className="min-w-44">
                  ...
                  >
                    <Globe2 />
                    {props.qaMode ? "Test application" : "Browser"}
                  </SurfaceMenuItem>
                  {!props.qaMode ? (
                    <>

...
import { prStatusIndicator, resolveThreadPr } from "./ThreadStatusIndicators.logic";
...
function useBranchToolbarBranchSelectorContent({
  className,
  environmentId,
  threadId,
  draftId,
  ...
  const sourceControlPresentation = getSourceControlPresentation(
  ...
  const branchNames = refs.map((refName) => refName.name);
  const branchByName = new Map(refs.map((refName) => [refName.name, refName] as const));
  ...
  const setThreadBranchFromEffect = useEffectEvent(setThreadBranch);
  useEffect(() => {
    if (
    ...
    setThreadBranchFromEffect(currentGitBranch, null);
  }, [activeThreadBranch, activeWorktreePath, currentGitBranch, effectiveEnvMode]);
  ...
  const fetchNextBranchPage = () => {
    if (!hasNextPage || isFetchingNextPage) {
      return;
    }
    branchRefState.loadNext();
  };
  const maybeFetchNextBranchPage = () => {
  ...
  const updateBranchListScrollFades = () => {
  ...
  const updateBranchListScrollFadesFromEffect = useEffectEvent(updateBranchListScrollFades);
  useLayoutEffect(() => {
    if (!isBranchMenuOpen) {
      return;
    }
    ...
    const frame = requestAnimationFrame(() => {
      updateBranchListScrollFadesFromEffect();
      nestedFrame = requestAnimationFrame(updateBranchListScrollFadesFromEffect);
    });
    return () => {
      cancelAnimationFrame(frame);
      cancelAnimationFrame(nestedFrame);
    };
  }, [deferredTrimmedBranchQuery, filteredBranchPickerItems.length, isBranchMenuOpen]);
  useEffect(() => {
    if (!isBranchMenuOpen) {
      return;
    }
  ...
  const maybeFetchNextBranchPageFromEffect = useEffectEvent(maybeFetchNextBranchPage);
  useEffect(() => {
    maybeFetchNextBranchPageFromEffect();
  }, [refs.length]);
...
export function BranchToolbarBranchSelector(props: BranchToolbarBranchSelectorProps) {
  return <>{useBranchToolbarBranchSelectorContent(props)}</>;
}

...
export function SkillInlineText(props: { text: string; skills: ReadonlyArray<InlineSkill> }) {
  ...
  const skillsByName = new Map(props.skills.map((skill) => [skill.name, skill] as const));
  let cursor = 0;
...
export function SkillInlineMarkdownChildren(props: {
  children: ReactNode;
  ...
  return (
    <>
      {Children.map(children, (child) => {
        ...
        if (child.type === "code" || child.type === "a") return child;
        if (!("children" in child.props)) return child;
        return cloneElement(
          child,
          undefined,
          <SkillInlineMarkdownChildren skills={skills}>
            {child.props.children}
          </SkillInlineMarkdownChildren>,
        );
      })}
    </>
  );

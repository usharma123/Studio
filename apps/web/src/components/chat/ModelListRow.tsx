...
export const ModelListRow = function ModelListRow(props: {
  index: number;
  model: ModelEsque;
      ...
      <div className="min-w-0 flex-1 text-left">
        <div className="flex min-w-0 items-center gap-2">
          <div className="min-w-0 truncate text-xs font-medium leading-snug">
            {props.flags.useTriggerLabel
              ? getTriggerDisplayModelLabel(props.model)
              : getDisplayModelName(
                  props.model,
                  props.flags.preferShortName ? { preferShortName: true } : undefined,
                )}
          </div>
          {props.flags.isSelected ? (
            <CheckIcon className="size-3.5 shrink-0 text-blue-400" />
          ) : null}
          {props.flags.showNewBadge ? (
            <span
      ...
      <div className="flex shrink-0 items-center gap-1.5">
        ...
        <Tooltip>
            ...
            render={
              <Button
                ...
                className={cn(
                  ...
                  props.flags.isFavorite && "text-foreground opacity-100",
                )}
                onClick={(event) => {
                ...
                disabled={Boolean(props.disabledReason)}
                aria-label={props.flags.isFavorite ? "Remove from favorites" : "Add to favorites"}
              >
                <StarIcon
                  className={cn(
                    "size-3.5 sm:size-3",
                    props.flags.isFavorite && "fill-current text-yellow-500",
                  )}
                />
              </Button>
            }
          ...
          <TooltipPopup side="top" align="center">
            {props.flags.isFavorite ? "Remove from favorites" : "Add to favorites"}
          </TooltipPopup>
        </Tooltip>
      </div>
    </ComboboxItem>

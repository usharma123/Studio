...
const useModelPickerContent = function useModelPickerContent(props: {
  ...
  const keybindings = (() => providedKeybindings ?? [])();
  const updateSettings = useUpdateClientSettings();
  const focusSearchInput = () => {
    searchInputRef.current?.focus({ preventScroll: true });
  };
  const focusSearchInputFromEffect = useEffectEvent(() => {
    searchInputRef.current?.focus({ preventScroll: true });
  });
  ...
  const lockedDisabledInstanceIds = (() => {
    if (!isLocked) {
      return undefined;
    }
  ...
  const instanceOrder = (() => instanceEntries.map((entry) => entry.instanceId))();
  ...
  const handleModelSelect = (modelSlug: string, instanceId: ProviderInstanceId) => {
  ...
  const handleModelSelectFromEffect = useEffectEvent(handleModelSelect);
  const toggleFavorite = (instanceId: ProviderInstanceId, model: string) => {
  ...
  const modelJumpModelKeys = (() => [...modelJumpCommandByKey.keys()])();
  const allModelKeys = ((): string[] =>
    flatModels.map((model) => `${model.instanceId}:${model.slug}`))();
  const filteredModelKeys = ((): string[] =>
    filteredModels.map((model) => `${model.instanceId}:${model.slug}`))();
  const filteredModelByKey = ((): ReadonlyMap<string, ModelPickerItem> =>
    new Map(
      filteredModels.map((model) => [`${model.instanceId}:${model.slug}`, model] as const),
    ))();
  const updateModelListScrollFades = () => {
  ...
  const updateModelListScrollFadesFromEffect = useEffectEvent(updateModelListScrollFades);
  const modelJumpShortcutContext = (() =>
    ({
      terminalFocus: false,
      terminalOpen: props.terminalOpen,
      modelPickerOpen: true,
    }) as const)();
  const modelJumpLabelByKey = ((): ReadonlyMap<string, string> => {
  ...
  useEffect(() => {
    const onWindowKeyDown = (event: globalThis.KeyboardEvent) => {
      ...
      handleModelSelectFromEffect(slug, instanceId);
    };
    window.addEventListener("keydown", onWindowKeyDown, true);
    return () => {
      window.removeEventListener("keydown", onWindowKeyDown, true);
    };
  }, [keybindings, modelJumpModelKeys, modelJumpShortcutContext]);
  useLayoutEffect(() => {
    let nestedFrame = 0;
    const frame = window.requestAnimationFrame(() => {
      setShowTopScrollFade(false);
      setShowBottomScrollFade(filteredModelKeys.length > 5);
      updateModelListScrollFadesFromEffect();
...
export function ModelPickerContent(props: Parameters<typeof useModelPickerContent>[0]) {
  return useModelPickerContent(props);
}

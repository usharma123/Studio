...
const ComposerFooterModeControls = function ComposerFooterModeControls(props: {
...
const ComposerFooterPrimaryActions = function ComposerFooterPrimaryActions(props: {
  activeContextWindow: ReturnType<typeof deriveLatestContextWindowSnapshot>;
  activeThreadProviderDisplayName: string | null;
  actionLayout: ComponentProps<typeof ComposerPrimaryActions>["layout"];
  actionState: ComponentProps<typeof ComposerPrimaryActions>["state"];
  ...
  return (
    <>
      ...
      {props.actionState.isPreparingWorktree ? (
...
// Component
// --------------------------------------------------------------------------
const useChatComposerContent = function useChatComposerContent(props: ChatComposerProps) {
  const {
    composerDraftTarget,
    environmentId,
    routeKind,
  ...
  const lockedContinuationGroupKey = ((): string | null => {
  ...
  const selectedProviderStatus = (() => selectedProviderEntry?.snapshot ?? null)();
  const selectedProviderModels = (() => selectedProviderEntry?.models ?? [])();
  const composerPromptInjectionState = (() => getComposerPromptInjectionState(prompt))();
  const composerProviderState = (() =>
    getComposerProviderState({
  ...
  const composerProviderControls = (() => ({
  ...
  const activeThreadProviderDisplayName = (() => {
  ...
  const composerMenuItems: ComposerCommandItem[] = (() => {
    if (!composerTrigger || props.qaMode) return [];
  ...
  const composerPreviewAnnotationIdSet = new Set(
    composerPreviewAnnotations.map((annotation) => annotation.id),
  );
  const unannotatedComposerImages = composerImages.filter(
    (image) => !composerPreviewAnnotationIdSet.has(image.id),
  );
  ...
  const blurMobileComposerAfterSend = () => {
  ...
  const shouldBlurMobileComposerOnSubmit = () => {
  ...
  const submitComposer = (event?: { preventDefault: () => void }) => {
    onSend(event);
  ...
  const onComposerCommandKey = (
    ...
    if (!props.qaMode && key === "Tab" && event.shiftKey) {
      toggleInteractionMode();
      return true;
    }
  ...
  const handleImplementPlanInNewThreadPrimaryAction = () => {
    void onImplementPlanInNewThread();
  };
  const scheduleComposerCollapseCheck = () => {
    if (!isMobileViewport) {
      return;
    }
        ...
        >
              ...
              <div className="px-3 pb-3 sm:px-4">
                ...
                >
                  ...
                  {activePendingProgress?.activeQuestion?.multiSelect ? (
                    <ComposerPrimaryActions
                      layout={{ compact: true, preserveComposerFocusOnPointerDown: true }}
                      state={{
          ...
          >
            ...
            {!isComposerCollapsedMobile &&
              !isComposerApprovalState &&
              pendingUserInputs.length === 0 &&
              unannotatedComposerImages.length > 0 && (
                <div className="mb-3 flex flex-wrap gap-2">
                  {unannotatedComposerImages.map((image) => (
                    <div
                      key={image.id}
            ...
            <div className="relative">
              <ComposerPromptEditor
                ...
                skills={props.qaMode ? [] : (selectedProviderStatus?.skills ?? [])}
                  ...
                  isComposerApprovalState
                    ...
                    : activePendingProgress
                      ...
                      : showPlanFollowUpPrompt && activeProposedPlan
                        ...
                        : environmentUnavailable
                          ? `${environmentUnavailable.label}: ${connectionStatusText(environmentUnavailable.connection)}`
                          : props.qaMode
                            ? "Ask the release assistant or attach QA evidence"
                            : phase === "disconnected"
...
export function ChatComposer(props: ChatComposerProps) {
  return useChatComposerContent(props);
}

...
describe("MessagesTimeline", () => {
  it("renders a caller-provided empty state before the first turn", async () => {
    const { MessagesTimeline } = await import("./MessagesTimeline");
    const markup = renderToStaticMarkup(
      <MessagesTimeline
        {...buildProps()}
        emptyState={<div data-testid="new-task-welcome">Welcome to Repro</div>}
        timelineEntries={[]}
      />,
    );

    expect(markup).toContain('data-testid="new-task-welcome"');
    expect(markup).toContain("Welcome to Repro");

...
// ---------------------------------------------------------------------------
export const MessagesTimeline = function MessagesTimeline({
  isWorking,
  ...
  const onToggleTurnFold = (turnId: TurnId) => {
  ...
  const onToggleWorkGroup = (groupId: string, anchorElement?: HTMLElement) => {
  ...
  const minimapItems = (() => deriveTimelineMinimapItems(rows))();
  const [timelineViewportElement, setTimelineViewportElement] = useState<HTMLDivElement | null>(
    null,
  );
  const [minimapHasPersistentGutter, setMinimapHasPersistentGutter] = useState(false);
  const handleAnchorReady = (info: { anchorIndex: number | undefined }) => {
    if (anchorMessageId !== null && info.anchorIndex !== undefined) {
      onAnchorReady(anchorMessageId, info.anchorIndex);
    }
  };
  const handleAnchorSizeChanged = (size: number) => {
  ...
  const handleScrollFromEffect = useEffectEvent(handleScroll);
  useEffect(() => {
    const frame = requestAnimationFrame(() => {
      handleScrollFromEffect();
    });
...
function renderTimelineItem({ item }: { item: MessagesTimelineRow }) {
  return (
...
function TimelineMinimap({
    ...
    resolvedActiveIndex === null
      ...
      : resolvedActiveIndex === 0
        ...
        : resolvedActiveIndex === items.length - 1
          ? "-100%"
          : "-50%";
  const resolveActiveIndexFromPointer = (event: MouseEvent<HTMLElement>) => {
  ...
  const updateActiveIndexFromPointer = (event: MouseEvent<HTMLElement>) => {
...
const TimelineRowContent = function TimelineRowContent({ row }: { row: TimelineRow }) {
  return (
    <div
      className={cn(
...
const WorkGroupSection = function WorkGroupSection({
  groupedEntries,
}: {
...
const AssistantChangedFilesSection = function AssistantChangedFilesSection({
  turnSummary,
  routeThreadKey,
  resolvedTheme,
  onOpenTurnDiff,
...
const UserMessageElementContextChip = function UserMessageElementContextChip(props: {
...
const CollapsibleUserMessageBody = function CollapsibleUserMessageBody(props: {
  text: string;
...
const UserMessageBody = function UserMessageBody(props: {
  text: string;
  terminalContexts: ParsedTerminalContextEntry[];
  skills: ReadonlyArray<Pick<ServerProviderSkill, "name" | "displayName">>;
  markdownCwd: string | undefined;
...
function useStableRows(rows: MessagesTimelineRow[]): MessagesTimelineRow[] {
  const [snapshot, setSnapshot] = useState(() => ({
    source: rows,
    stable: computeStableMessagesTimelineRows(rows, {
      byId: new Map<string, MessagesTimelineRow>(),
      result: [],
    }),
  }));
  if (snapshot.source !== rows) {
    const stable = computeStableMessagesTimelineRows(rows, snapshot.stable);
    setSnapshot({ source: rows, stable });
    return stable.result;
  }
  return snapshot.stable.result;
}
// ---------------------------------------------------------------------------
// Pure helpers
// ---------------------------------------------------------------------------
...
const SimpleWorkEntryRow = function SimpleWorkEntryRow(props: {
  workEntry: TimelineWorkEntry;
  workspaceRoot: string | undefined;
}) {
  ...
  const rowToggleProps = canExpand
    ? {
        ...
        onClick: (event: MouseEvent<HTMLDivElement>) => {
          if ((event.target as Element).closest("[data-work-entry-expanded-body]")) return;
  ...
  return (
    ...
    >
      ...
      {expanded && canExpand && expandedBody ? (
        <div
          data-work-entry-expanded-body="true"

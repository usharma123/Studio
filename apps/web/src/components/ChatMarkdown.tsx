...
import { SkillInlineMarkdownChildren } from "./chat/SkillInlineText";
...
function transformMarkdownUrl(href: string): string {
...
type HighlightedCode = ReturnType<DiffsHighlighter["codeToTokens"]>;

const highlightedCodeCache = new LRUCache<HighlightedCode>(
...
function estimateHighlightedSize(code: string): number {
  return code.length * 8;
}

function renderHighlightedCode(highlightedCode: HighlightedCode): ReactNode {
  let lineStart = 0;
  return (
    <pre
      className="shiki"
      style={{ color: highlightedCode.fg, backgroundColor: highlightedCode.bg }}
    >
      <code>
        {highlightedCode.tokens.map((line, lineIndex) => {
          const currentLineStart = lineStart;
          lineStart += line.reduce((length, token) => length + token.content.length, 0) + 1;
          return (
            <span className="line" key={currentLineStart}>
              {line.map((token) => {
                const fontStyle = token.fontStyle ?? 0;
                return (
                  <span
                    key={`${currentLineStart}:${token.offset}`}
                    style={{
                      color: token.color,
              ...
              {lineIndex < highlightedCode.tokens.length - 1 ? "\n" : null}
            </span>
          );
        })}
      </code>
...
function SuspenseShikiCodeBlock({
  ...
  const cachedHighlightedCode = !isStreaming ? highlightedCodeCache.get(cacheKey) : null;

  if (cachedHighlightedCode != null) {
    return (
      <div className="chat-markdown-shiki">{renderHighlightedCode(cachedHighlightedCode)}</div>
    );
  }
...
function UncachedShikiCodeBlock({
  ...
  const highlightedCode = (() => {
    try {
      return highlighter.codeToTokens(code, { lang: language, theme: themeName });
    } catch (error) {
      ...
      return highlighter.codeToTokens(code, { lang: "text", theme: themeName });
    }
  })();

  useEffect(() => {
    if (!isStreaming) {
      highlightedCodeCache.set(cacheKey, highlightedCode, estimateHighlightedSize(code));
    }
  }, [cacheKey, code, highlightedCode, isStreaming]);

  return <div className="chat-markdown-shiki">{renderHighlightedCode(highlightedCode)}</div>;
}
...
function MarkdownLinkFavicon({ host }: { host: string }) {
...
interface MarkdownComponentsContext {
  readonly text: string;
  readonly skills: NonNullable<ChatMarkdownProps["skills"]>;
  ...
  readonly openInPreferredEditor: ReturnType<typeof useOpenInPreferredEditor>;
  readonly openExternalLinkInPreview: (url: string) => ReturnType<typeof openUrlInPreview>;
  readonly openMarkdownFileInPreview: (path: string) => ReturnType<typeof openFileInPreview>;
}
...
}: MarkdownComponentsContext): Components {
  return {
    p({ node: _node, children, ...props }) {
      return (
        <p {...props}>
          <SkillInlineMarkdownChildren skills={skills}>{children}</SkillInlineMarkdownChildren>
        </p>
      );
    },
...
function ChatMarkdown({
  ...
  const fileLinkParentSuffixByPath = (() => {
    const filePaths = [...markdownFileLinkMetaByHref.values()].map((meta) => meta.filePath);
    return buildFileLinkParentSuffixByPath(filePaths);
  })();
  const openExternalLinkInPreview = (url: string) => {
    if (!threadRef) {
  ...
  const openMarkdownFileInPreview = (path: string) => {

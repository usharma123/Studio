...
it.layer(NodeServices.layer)("PairingGrantStore.layer", (it) => {
  ...
  it.effect("seeds the desktop bootstrap credential as a reusable grant", () =>
    Effect.gen(function* () {
      ...
      expect(first.subject).toBe("local:root");
  ...
  it.effect("uses the trusted desktop profile as the authenticated subject", () =>
    Effect.gen(function* () {
      const bootstrapCredentials = yield* PairingGrantStore.PairingGrantStore;
      const grant = yield* bootstrapCredentials.consume("desktop-bootstrap-token");

      expect(grant.subject).toBe("local:qa:maker");
      expect(grant.method).toBe("desktop-bootstrap");
      expect(grant.scopes).toEqual(["qa:read", "qa:make", "qa:chat", "preview:operate"]);
    }).pipe(
      Effect.provide(
        makePairingGrantStoreLayer({
          desktopBootstrapToken: "desktop-bootstrap-token",
  ...
  it.effect("keeps the approver bootstrap grant out of maker and workspace scopes", () =>
    Effect.gen(function* () {
      const bootstrapCredentials = yield* PairingGrantStore.PairingGrantStore;
      const grant = yield* bootstrapCredentials.consume("desktop-bootstrap-token");

      expect(grant.subject).toBe("local:qa:approver");
      expect(grant.scopes).toEqual(["qa:read", "qa:approve", "qa:chat", "preview:operate"]);
      expect(grant.scopes).not.toContain("orchestration:read");
      expect(grant.scopes).not.toContain("orchestration:operate");
      expect(grant.scopes).not.toContain("terminal:operate");
    }).pipe(
      Effect.provide(
        makePairingGrantStoreLayer({
          desktopBootstrapToken: "desktop-bootstrap-token",
          desktopDevelopmentProfile: "qa:approver",
        }),
      ),
    ),
  );

...
const makeWithOptions = Effect.fn("McpSessionRegistry.make")(function* (
  ...
  const qaIam = yield* Effect.serviceOption(QaIam.QaIam);
  ...
  const issue: McpSessionRegistryShape["issue"] = Effect.fn("McpSessionRegistry.issue")(
    function* (request) {
      ...
      const qaContext = Option.isSome(qaIam)
        ? yield* qaIam.value.resolveConversationContext(request.threadId).pipe(Effect.option)
        : Option.none();
      const capabilities = new Set<McpInvocationContext.McpCapability>(["preview"]);
      if (Option.isSome(qaContext)) {
        capabilities.add("qa:read");
        if (qaContext.value.capabilities.includes("qa:make")) {
      ...
      const scope: McpInvocationContext.McpInvocationScope = {
        ...
        capabilities,
        ...(Option.isSome(qaContext)
          ? {
              qaReleaseThreadId: ThreadId.make(qaContext.value.releaseThreadId),
              qaPrincipalSubject: qaContext.value.principal.subject,
            }
          : {}),
        issuedAt,
        expiresAt,

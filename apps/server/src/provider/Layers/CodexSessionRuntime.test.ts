...
describe("buildTurnStartParams", () => {
  ...
  it("defers to the configured QA permission profile without legacy sandbox overrides", () => {
    const params = Effect.runSync(
      buildTurnStartParams({
        threadId: "provider-thread-qa",
        runtimeMode: "approval-required",
    ...
    NodeAssert.deepStrictEqual(params, {
      threadId: "provider-thread-qa",
      approvalPolicy: "never",
      input: [{ type: "text", text: "Review the release evidence" }],
    });
  });
});
...
describe("openCodexThread", () => {
  it.effect("uses the configured QA permission profile without a sandbox override", () =>
    Effect.gen(function* () {
      ...
      const client = {
        request: <M extends "thread/start" | "thread/resume">(
          ...
          return Effect.succeed(
            makeThreadOpenResponse("qa-thread") as CodexRpc.ClientRequestResponsesByMethod[M],
          );
        },
      };
      ...
      NodeAssert.deepStrictEqual(calls, [
        {
          method: "thread/start",
          payload: { cwd: "/tmp/project", approvalPolicy: "never" },
        },
      ]);
    }),
  );

...
export interface CodexSessionRuntimeOptions {
  ...
  /** Use the app-server's configured permission profile instead of legacy sandbox overrides. */
...
function buildThreadStartParams(input: {
  ...
  return {
    cwd: input.cwd,
    approvalPolicy: input.useConfiguredPermissionProfile ? "never" : config.approvalPolicy,
    ...(!input.useConfiguredPermissionProfile ? { sandbox: config.sandbox } : {}),
...
> =>
  Effect.gen(function* () {
    ...
    // Keep newer app-server notification methods observable even when this
    // checkout's generated schema predates the locally installed Codex CLI.
    // Typed methods continue through the normalized queue below; only truly
    // unknown methods reach this forward-compatible fallback.
    ...
    yield* client.handleUnknownServerRequest((method, requestPayload) => {
      if (method === "currentTime/read") {
        return Clock.currentTimeMillis.pipe(
          Effect.map((currentTimeMillis) => ({
            currentTimeAt: Math.floor(currentTimeMillis / 1_000),
          })),
        );
      }
      if (method === "item/permissions/requestApproval") {
        return emitEvent({
          ...
          method,
          payload: {
            disposition: "denied-by-client",
            reason: "Studio does not yet expose permission escalation UX.",
      ...
      if (method === "mcpServer/elicitation/request") {
        return emitEvent({
          ...
          method,
          payload: {
            disposition: "cancelled-by-client",
            reason: "Studio does not yet expose MCP elicitation UX.",
            request: requestPayload,
          },
        }).pipe(Effect.as({ action: "cancel" }));
      }
    ...
    const start = Effect.fn("CodexSessionRuntime.start")(function* () {
      ...
      const opened = yield* openCodexThread({
        ...
        ...(options.useConfiguredPermissionProfile ? { useConfiguredPermissionProfile: true } : {}),
      });
      ...
      sendTurn: (input) =>
        Effect.gen(function* () {
          ...
          const params = yield* buildTurnStartParams({
            ...
            ...(input.effort ? { effort: input.effort } : {}),
            ...(input.interactionMode ? { interactionMode: input.interactionMode } : {}),
            ...(options.useConfiguredPermissionProfile

...
it.layer(NodeServices.layer)("effect-codex-app-server client", (it) => {
  ...
  it.effect("routes known but unregistered server requests through the compatibility handler", () =>
    Effect.gen(function* () {
      const compatibilityRequests = yield* Ref.make<Array<{ method: string; params: unknown }>>([]);
      const handle = yield* makeHandle({
        CODEX_APP_SERVER_TEST_KNOWN_UNHANDLED_REQUEST: "1",
      });
      const scope = yield* Scope.make();
      const clientLayer = CodexClient.layerChildProcess(handle);
      const context = yield* Layer.buildWithScope(clientLayer, scope);
      ...
      yield* Effect.gen(function* () {
        ...
        yield* client.handleUnknownServerRequest((method, params) =>
          Ref.update(compatibilityRequests, (current) => [...current, { method, params }]).pipe(
        ...
        yield* client.request("skills/list", { cwds: [process.cwd()] });
      }).pipe(Effect.provide(context), Effect.ensuring(Scope.close(scope, Exit.void)));

      assert.deepEqual(yield* Ref.get(compatibilityRequests), [
        {

...
export const make = Effect.fn("effect-codex-app-server/CodexAppServerClient.make")(function* (
  ...
  const dispatchNotification = (
    ...
    const dispatchUnknownNotification = () =>
    ...
    if (schema) {
      if (handlers.length === 0) {
        return dispatchUnknownNotification();
      }
      return decodeNotificationPayload(notification.method, schema, notification.params).pipe(
        ...
        Effect.catch(dispatchUnknownNotification),
      );
    }
  ...
  const dispatchRequest = (
    ...
    if (request.method in CodexRpc.SERVER_REQUEST_PARAMS) {
      ...
      // Generated bindings can know about a method before this client has
      // product UX for it. Give the compatibility handler the same chance to
      // answer those requests as it gets for entirely new protocol methods.
      if (!handler && unknownRequestHandler) {

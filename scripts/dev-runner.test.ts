  ...
  describe("createDevRunnerEnv", () => {
    ...
    it.effect("defaults T3CODE_HOME to ~/.codex-studio when not provided", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.T3CODE_HOME, path.resolve(NodeOS.homedir(), ".codex-studio"));
      }),
    );
    ...
    it.effect("isolates desktop profile state while preserving the shared QA database", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const env = yield* createDevRunnerEnv({
          mode: "dev:desktop",
          baseEnv: {
            T3CODE_QA_DATABASE_URL: "postgresql://qa-shared.example/test",
          },
          serverOffset: 12,
          webOffset: 12,
          t3Home: "/tmp/t3",
        ...
        assert.equal(env.T3CODE_HOME, path.resolve("/tmp/t3/profiles/qa-maker"));

...
const DESKTOP_APP_ID = "com.codexstudio.desktop";
...
export function resolveDesktopProductName(version: string): string {
  return resolveDesktopUpdateChannel(version) === "nightly"
    ? "Studio (Nightly)"
    : (desktopPackageJson.productName ?? "Studio");
}
  ...
  if (platform === "mac") {
    buildConfig.mac = {
      ...
      protocols: [
        {
          name: "Studio",
          schemes: ["codex-studio", "codex-studio-dev"],
        },
      ],
      ...(macPasskeySigning
        ? {
...
const buildDesktopArtifact = Effect.fn("buildDesktopArtifact")(function* (
  ...
  const stagePackageJson: StagePackageJson = {
    name: "codex-studio-desktop",
    version: appVersion,
    buildVersion: appVersion,
    t3codeCommitHash: commitHash,
    private: true,
    packageManager: rootPackageJson.packageManager,
    description: "Studio desktop build",
    author: "Studio contributors",
...
const buildDesktopArtifactCli = Command.make("build-desktop-artifact", {
  ...
  Command.withDescription("Build a desktop artifact for Studio."),

...
import {
  ...
  type DesktopDevelopmentProfile as DesktopDevelopmentProfileValue,
...
export const DEFAULT_T3_HOME = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(NodeOS.homedir(), ".codex-studio"),
);
...
interface CreateDevRunnerEnvInput {
  ...
  readonly profile?: DesktopDevelopmentProfileValue;
}
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    const baseDir = yield* resolveBaseDir(t3Home);
    const resolvedBaseDir =
      profile === undefined
        ? baseDir
        : path.join(baseDir, "profiles", DESKTOP_PROFILE_SLUGS[profile]);
    ...
    if (profile !== undefined) {
      output.T3CODE_DEV_PROFILE = profile;
      output.T3CODE_DEV_INSTANCE = `desktop-${DESKTOP_PROFILE_SLUGS[profile]}`;
    } else {
      delete output.T3CODE_DEV_PROFILE;
    }
...
interface DevRunnerCliInput {
  ...
  readonly profile: DesktopDevelopmentProfileValue | undefined;
}
...
export function runDevRunnerWithInput(input: DevRunnerCliInput) {
  return Effect.gen(function* () {
    ...
    const selectedDevInstance =
      input.profile === undefined ? devInstance : `desktop-${DESKTOP_PROFILE_SLUGS[input.profile]}`;
    const { offset, source } = yield* resolveOffset({
      portOffset,
      devInstance: selectedDevInstance,
    });
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  profile: Flag.choice("profile", DesktopDevelopmentProfile.literals).pipe(
    Flag.withDescription("Trusted isolated desktop development principal."),
    Flag.optional,
    Flag.map(Option.getOrUndefined),
  ),
